...
import type {
  ...
  RegistryIndex,
  SpeedSweep,
} from "../../local-ai-registry/registry/schema/types"

export type * from "../../local-ai-registry/registry/schema/types"
...
function registryRoot(): string {
  ...
  const candidates = [
    ...
    path.join(process.cwd(), "packages", "local-ai-registry", "registry"),
    path.join(process.cwd(), "..", "local-ai-registry", "registry"),
    path.join(process.cwd(), "local-ai-registry", "registry"),
...
function readJson<T>(...parts: string[]): T {
  return JSON.parse(
    readFileSync(path.join(registryRoot(), ...parts), "utf8"),
  ) as T
}
...
export function getCompatibilityResult(id: string): CompatibilityResult | undefined {
  const row = dataset().index.recipes.find((candidate) => candidate.id === id)
  return row ? compatibilityResult(row) : undefined
}
...
export function queryCompatibility(
  ...
  const rows = matchingCompatibilityRows(filters).sort((left, right) => {
    ...
    return leftKeys[0].localeCompare(rightKeys[0], undefined, { numeric: true })
      || (primary === "hardware" ? left.hardware_count - right.hardware_count : 0)
      || leftKeys[1].localeCompare(rightKeys[1], undefined, { numeric: true })
      || (primary === "model" ? left.hardware_count - right.hardware_count : 0)
...
export function listSpeedSweeps(filters: Record<string, string>, pagination: Pagination) {
  ...
  const all = ids.flatMap((id) => {
    ...
    if (!sweep) return []
    if (!contains([sweep, getCompatibilityResult(sweep.recipe_id)], filters.q) || !equals(sweep.recipe_id, filters.recipe_id)) return []
    return [sweep]
  })
  return { data: all.slice(pagination.offset, pagination.offset + pagination.limit), total: all.length }
}
...
export function getEntityDetail(collection: string, id: string): RegistryRecord | undefined {
  ...
  if (collection === "speed-sweeps") {
    ...
    return {
      ...sweep,
      name: result ? `${result.model.name} on ${result.hardware.name}` : "Speed sweep",
      relationships: {
        hardware: result ? recordLink("hardware", result.hardware.id, result.hardware.name) : null,
        model: result ? recordLink("models", result.model.id, result.model.name) : null,

...
test("recipe browsing sorts by hardware or model before pagination", () => {
  ...
  const ordered = (items: typeof byHardware, primary: "hardware" | "model") => items.every((item, index) => {
    ...
    if (primary === "hardware") {
      const countOrder = previous.recipe.hardware_count - item.recipe.hardware_count
      if (countOrder !== 0) return countOrder < 0
    }
    const secondOrder = second[0].localeCompare(second[1], undefined, { numeric: true })
    if (secondOrder !== 0) return secondOrder < 0
    return primary === "model" ? previous.recipe.hardware_count <= item.recipe.hardware_count : true
  })
...
test("speed sweeps resolve human model and hardware context", () => {
  const sweeps = listSpeedSweeps({ q: "RTX Pro 6000 Blackwell" }, { limit: 10, offset: 0 })
  assert.ok(sweeps.total > 0)
  let matchedHardware = false
  for (const sweep of sweeps.data) {
    const result = getCompatibilityResult(sweep.recipe_id)
    assert.ok(result)
    if (/RTX Pro 6000 Blackwell/i.test(result.hardware.name)) matchedHardware = true
  }
  assert.equal(matchedHardware, true)

  const detail = getEntityDetail("speed-sweeps", sweeps.data[0].id)
  assert.ok(detail)

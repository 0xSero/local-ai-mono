...
ai_hardware_json() {
  ...
  if [[ $backend == "intel-xpu" ]] && command -v lspci >/dev/null 2>&1; then
    ...
    if [[ -n ${OMARCHY_AI_GPUS:-} ]]; then
      rows=$(nvidia-smi -i "$OMARCHY_AI_GPUS" --query-gpu="$query" --format=csv,noheader,nounits 2>/dev/null || true)
    else
      rows=$(nvidia-smi --query-gpu="$query" --format=csv,noheader,nounits 2>/dev/null || true)
    fi
    if [[ -z $rows ]]; then
